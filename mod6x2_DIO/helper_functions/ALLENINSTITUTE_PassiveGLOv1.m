...
for i = 1 : 5 : total_trials
    if (~floor(sum(task_data.orientation(i:i+3)==seq_go')/4) & ...
        ...
        task_data.rndctl                     = zeros(total_trials,1); task_data.rndctl(i:end-500) = 1; % randomized control sequences
        task_data.seqctl                     = zeros(total_trials,1); task_data.seqctl(end-499:end) = 1; % sequenced control with alternating [g g g g, l l l l, etc.]
    end
