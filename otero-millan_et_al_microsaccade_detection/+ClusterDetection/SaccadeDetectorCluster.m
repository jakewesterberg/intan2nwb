classdef SaccadeDetectorCluster < ClusterDetection.SaccadeDetector
    ...
    methods
        ...
        function [sac stats] = FindSaccades( this, eyeRecording ) 
            ...
            peakIsSac = zeros(size(clusteridx));
            for i=1:(max(clusteridx)-1)
                props = this.GetSaccadeProps( eyeRecording, peaks(clusteridx==i,:));
                if ( mean(props(:,enum.displacement)) > 0.2 )
                    peakIsSac(clusteridx==i) = 1;
                end
            end
            
            
            sac = peaks(find(peakIsSac), :);
            sac = this.GetSaccadeProps( eyeRecording, sac  );
        end
        
        function peaks = FindPeaks( this, eyeRecording )
            
            this.MINIPI = round(20*eyeRecording.samplerate/1000); % minimum inter peak interval
