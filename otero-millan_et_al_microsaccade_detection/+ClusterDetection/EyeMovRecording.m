classdef EyeMovRecording < ClusterDetection.DataDB
    ...
    methods
        ...
        function trials = GetTrials( this )
            ...
            time = this.time;
            % trials
            isInTrial = ([0;diff(time)]-median(diff(time))) < 0.1; % jumps in timestamps
            
            trialStartIdx = find(diff([0;isInTrial])>0);
            trialEndIdx = find(diff([isInTrial;0])<0);
                        
            % adjust the begining and end of trials
            trialStartIdx = trialStartIdx + round(this.TRIAL_CROP_BEGINING*this.samplerate);
            trialEndIdx = trialEndIdx - round(this.TRIAL_CROP_END*this.samplerate);
    ...
    methods ( Static = true );
        ...
        function yesNo = CreateYesNo( starts, stops, length )
            ...
            yesNo(starts) = yesNo(starts)+ 1;
            yesNo(stops) = yesNo(stops)  -1;
            yesNo = cumsum(yesNo);
        end
